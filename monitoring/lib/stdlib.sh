...
__log_init__() {
    ...
    unset _log_levels _loggers_level_map _log_dir_path
    declare -gA _log_levels _loggers_level_map _log_dir_path
...
set_log_file() {
    (($#)) || return
    [[ ${_log_dir_path} && ${_log_dir_path} -eq $1 ]] && return

    local file=$(echo $1 | rev | cut -d '/' -f1 | rev)
    local cnt=$((${#1} - ${#file} - 1)) # -1 for last slash(/)
    local path=$(echo $1 | cut -c -${cnt})

    if [ -z ${path} ]; then
        mkdir -p ${path} #  new folder
        if [ -z $1 ]; then
            echo "##### log file created #####" >$1 # new file
        fi
    fi

    _log_dir_path=$1
    #    echo "_log_dir_path=${_log_dir_path}"
}
...
_print_log() {
    ...
    if [[ $log_level_set ]]; then
        if [[ ${_log_dir_path} ]]; then
            ((log_level_set >= log_level)) && {
                printf '%(%Y-%m-%d:%H:%M:%S)T %-7s %s ' -1 "$in_level" "${BASH_SOURCE[2]}:${BASH_LINENO[1]}" >>${_log_dir_path}
                printf '%s\n' "$@" >>${_log_dir_path}
            }
        else
            ((log_level_set >= log_level)) && {
